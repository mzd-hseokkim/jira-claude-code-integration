...
- **Planning**: Jira 이슈 기반 기획 문서 자동 생성 (bkit PDCA 위임)
- **Design**: 코드베이스 분석 기반 설계 문서 생성 (bkit PDCA 위임)
- **Implementation**: 설계 문서 기반 구현 (bkit PDCA Do 위임)
- **Testing**: Playwright E2E / unit 테스트 실행 + Jira 리포트
- **Code Review**: bkit Gap 분석 + 코드 품질 분석 → Jira 코멘트로 자동 게시
...
| [bkit (Vibecoding Kit)](https://github.com/popup-studio-ai/bkit-claude-code) | Yes | 기획/설계 문서 품질, Gap 분석, 코드 리뷰 |
| [GitHub CLI (`gh`)](https://cli.github.com/) | PR 생성 시 | PR 생성/관리 |

## Quick Start

```bash
# 1. .mcp.json 설정 (프로젝트 루트에)
cp .mcp.json.example .mcp.json
# → .mcp.json의 env 값을 내 Jira 정보로 수정

# 2. Claude Code 실행 후 연결 확인
claude
> /jira
...
Jira MCP 서버([mcp-jira-server](https://www.npmjs.com/package/mcp-jira-server))를 Claude Code에 등록합니다. 두 가지 방법 중 선택:

**방법 A: 프로젝트별 `.mcp.json` (권장)**

프로젝트 루트에 `.mcp.json`을 만들어 Jira 설정:

```bash
cp .mcp.json.example .mcp.json
```

`.mcp.json`의 env 값을 수정:
```json
{
  "mcpServers": {
    "jira": {
...
> `.mcp.json`은 `.gitignore`에 포함되어 커밋되지 않습니다 (credentials 포함).

**방법 B: 전역 `settings.local.json`**

모든 프로젝트에서 같은 Jira 설정을 쓴다면 전역 등록:

```bash
# ~/.claude/settings.local.json 에 위와 동일한 내용 추가
```

### Step 3: bkit (Vibecoding Kit) 설치

기획/설계 문서 품질과 Gap 분석을 위해 [bkit](https://github.com/popup-studio-ai/bkit-claude-code)을 설치합니다.
...
> 자동 업데이트를 원하면 `~/.claude/settings.json`에 `"autoUpdate": true` 추가.
...
# === 태스크별 작업 (worktree에서 진행) ===
cd ../my-project_worktree/PROJ-123 # worktree로 이동
/jira-task start PROJ-123          # 1. 시작 (In Progress 전환)
/jira-task plan PROJ-123           # 2. 기획 문서 생성
/jira-task design PROJ-123         # 3. 설계 문서 생성
/jira-task impl PROJ-123           # 4. 구현 (설계 문서 기반)
/jira-task test PROJ-123           # 5. 테스트 (Playwright E2E + unit)
/jira-task review PROJ-123         # 6. 코드 리뷰
/jira-task pr PROJ-123             # 7. PR 생성
/jira-task done PROJ-123           # 8. 완료 (상태 전이 + 리포트)

# === 스프린트 리포트 ===
/jira-task report                  # 스프린트 전체 진행 상황
```
...
| 기획 | `plan` | TASK-ID | `docs/plan/<ID>.plan.md` | 이슈 조회, 코멘트 (bkit PDCA 위임) |
| 설계 | `design` | TASK-ID | `docs/design/<ID>.design.md` | 이슈 조회, 코멘트 (bkit PDCA 위임) |
| 구현 | `impl` | TASK-ID | 코드 변경 | 이슈 조회, 코멘트 (bkit PDCA Do 위임) |
| 테스트 | `test` | TASK-ID | `docs/test/<ID>.test-report.md` | 이슈 조회, 코멘트, 첨부파일 |
| 리뷰 | `review` | TASK-ID | 리뷰 리포트 | 코멘트 (bkit Gap 분석 + 코드 분석) |
| PR | `pr` | TASK-ID | GitHub PR | 코멘트, In Review 전환 |
| 완료 | `done` | TASK-ID | 완료 리포트 | 코멘트, Done 전환 |
| 현황 | `report` | - | `docs/reports/status-*.md` | 내 이슈 검색 (스프린트 or 프로젝트) |
...
│   ├── jira-task-init/          # init: 스프린트 일괄 초기화
...
│   └── jira-task-report/        # report: 스프린트 리포트
│
...
└── templates/                   # Document templates
    └── report.template.md      # 현황 리포트 (plan/design은 bkit 위임)
```
