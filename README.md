...
- **Merge**: 로컬 병합 (merge commit, squash, rebase 전략 선택) + worktree 정리 안내
- **Pull Request**: `gh` CLI로 PR 생성 + Jira에 PR 링크 자동 게시
- **Completion**: 상태 전이 + 완료 리포트 Jira 게시
...
> /jira-task merge                   # 로컬 병합 (worktree에서, 이후 세션 종료)

# 6. worktree 세션 종료 후 메인 레포로 이동
> cd ../your-project
> /jira-task pr                      # PR 생성 (remote push + GitHub PR)
> /jira-task done                    # 완료 처리 (상태 전이 + 리포트)
```
...
| `/jira-task merge <TASK-ID>` | 로컬 병합 (worktree 안에서 실행, 전략 선택 + Jira 전환) |
| `/jira-task pr <TASK-ID>` | PR 생성 + Jira에 PR 링크 게시 (메인 레포에서 실행) |
| `/jira-task done <TASK-ID>` | 완료 처리 (상태 전이 + 완료 리포트) |
...
/jira-task merge                   # 7. 로컬 병합 (전략 선택 → merge → worktree 정리 안내)
# ↑ worktree 세션 종료 후 메인 레포로 이동
cd ../my-project                   # 메인 레포로 이동
/jira-task pr                      # 8. PR 생성 (feature 브랜치 push + GitHub PR)
/jira-task done                    # 9. 완료 (상태 전이 + 완료 리포트)
...
remote가 설정되지 않은 git 프로젝트에서는 `pr` 단계를 건너뛰고 `merge` → `done` 순으로 처리합니다.
...
# ↑ worktree 세션 종료 후 메인 레포로 이동
cd ../my-project
/jira-task done                    # 완료 처리 (상태 전이 + 완료 리포트)
```

`merge` 실행 시 병합 전략을 선택합니다. 병합은 항상 **메인 레포**에서 이루어집니다 (`.jira-context.json`의 `repoRoot` 참조):
...
**실행 위치**: worktree 안에서 실행하지만 병합은 항상 **메인 레포**에서 이루어집니다.
`.jira-context.json`의 `repoRoot`를 읽어 메인 레포 경로를 정확히 참조하기 때문입니다.

**worktree 정리**: 병합 완료 후 Claude Code가 worktree 디렉토리 안에서 실행 중이므로 직접 삭제가 불가능합니다. 스킬이 삭제 명령을 안내해주므로 **세션 종료 후** 메인 레포에서 수동으로 실행하세요.

> **참고**: remote origin이 있는 경우, `merge` 이후 feature 브랜치(feature/TASK-ID)는 PR 생성을 위해 보존됩니다. PR 머지 후 삭제하세요.

### 개별 단계 설명

| 단계 | 커맨드 | 실행 위치 | 입력 | 출력 | Jira 연동 |
|------|--------|-----------|------|------|-----------|
| 초기화 | `init` | 메인 레포 | 태스크 수 | worktree 생성 | 이슈 검색, 코멘트 |
| 시작 | `start` | worktree | TASK-ID | 브랜치 생성, README | 이슈 조회, In Progress 전환, 코멘트 |
| 기획 | `plan` | worktree | TASK-ID | `docs/plan/<ID>.plan.md` | 이슈 조회, 코멘트, **첨부파일** |
| 설계 | `design` | worktree | TASK-ID | `docs/design/<ID>.design.md` | 이슈 조회, 코멘트, **첨부파일** |
| 구현 | `impl` | worktree | TASK-ID | 코드 변경 | 이슈 조회, 코멘트 |
| 테스트 | `test` | worktree | TASK-ID | `docs/test/<ID>.test-report.md` | 이슈 조회, 코멘트, **첨부파일** (리포트+스크린샷) |
| 리뷰 | `review` | worktree | TASK-ID | `docs/review/<ID>.review.md` | 코멘트 (Gap 분석 + 코드 리뷰), **첨부파일** |
| 병합 | `merge` | worktree | TASK-ID | 로컬 병합 완료, worktree 정리 안내 | 코멘트, In Review 전환 |
| PR | `pr` | 메인 레포 | TASK-ID | GitHub PR | 코멘트 |
| 완료 | `done` | 메인 레포 | TASK-ID | 완료 리포트 | 코멘트, Done 전환 |
| 현황 | `report` | 어디서든 | - | `docs/reports/status-*.md` | 내 할당 이슈 검색 |
| 상태 | `status` | 어디서든 | - | 터미널 출력 | 활성 태스크 조회 |
